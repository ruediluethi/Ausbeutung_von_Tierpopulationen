...
\begin{document}
	...
	Werden diese zwei Gleichungen mit den allgemein bekannten Lotka-Volterra-Gleichungen \cite{LotkaVolterra} verglichen, ist leicht zu erkennen, dass diese damit identisch sind.
	...
	Weiter wird untersucht, ob ein Schwingungsverhalten wie in Abschnitt \ref{sec:Raeuber_Beute} beschrieben, auch bei einem zellulären Automaten zu beobachten ist. Als Grundlage für die Implementation dient der zelluläre Automat Wator \cite{Wator}.
	...
	Die Fische sowie die Haie werden jeweils in einem separaten Durchlauf berechnet. Es werden also zuerst alle Fische, danach alle Haie berechnet. Damit keiner der Werte in der Matrix überschrieben wird, werden alle neu berechneten Werte in eine neue leere Matrix gespeichert.
	...
	Für die Simulation wurde eine \(128 \times 128\) Matrix verwendet. Als Ausgangslage wurden jeweils 128 Fische sowie 128 Haie an zufälliger Position verwendet. In den ersten 500 Zyklen (siehe Abbildung \ref{fig:wator_diagram_100}) scheint das Verhalten noch recht zufällig:
	\begin{figure}[!h]
  		\centering
